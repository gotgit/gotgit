...
其中函数 skip_commit 也是在 git-filter-branch 脚本中已经定义好的。该函数的作用就是将传递给提交过滤器脚本的参数 "<tree_id> -p parent1 -p parent2 ..." 进行处理，形成 "parent1 parent2" 的输出。参见 Git 命令脚本 :file:`\`git --exec-path\`/git-filter-branch` 中相关函数。
