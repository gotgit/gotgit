...
* 上图中的图示1，可以用圆圈代表功能1的历次提交，用三角代替功能2的历次提交。因为所有开发者都在主线上工作，所以提交混杂在一起。
...
* 图例3中可以看出包括功能2在内的所有功能和模块都继续提交，但是提交的分支可能各不相同。功能2的开发者将代码提交到特性分支上，其他开发者还提交到主线上。
...
在使用 CVS 或者 Subversion 等版本控制系统建立分支相对不太方便，因为分支建立需要相关授权。Git 的分支管理就方便多了，一是开发者可以在本地版本库随心所欲的创建分支，二是管理员可以对共享版本库进行设置允许开发者创建特定名称的分支，这样开发者的本地分支可以推送到服务器实现数据的备份。关于 Git 服务器的分支授权参照本书第5篇的Gitolite 服务器架设相关章节。
...
* 用getopt做参数解析。
  ...
  即对参数解析的框架进行改造，以便实现更灵活的命令行处理。在 v1.0 版本中，程序内部解析参数使用了简单的字符串比较，非常不灵活。从源文件 `src/main.c` 中可以当前实现的局限。
...
最终决定由开发者 user2 负责多语种支持的功能，由开发者 user1 负责用getopt做参数解析的功能。
...
开发者 user1 负责用getopt做参数解析的功能，因为这个功能用到 `getopt` 函数，于是将这个分支命名为 `user1/getopt` 。开发者 user1 使用 `git branch` 命令创建该特性分支。

* 确保工作在开发者 user1 的工作区中。
...
有没有简单的操作，在创建分支后立即切换到新分支上呢？是的，Git 提供了这样一个命令，将上述两条命令所执行的操作一次性完成。用法如下：
...
* 执行下面的命令，用里程碑 `jx/v2.0` 标记的内容（已实现用getopt做参数解析的功能）替换暂存区和工作区。
...
* 比较暂存区和HEAD的文件差异，可以看到为实现用getopt做参数解析功能而对代码的改动。
